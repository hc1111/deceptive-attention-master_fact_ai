...
from sklearn.metrics importstthews_corrcoef, f1_score
...
class PronounProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, limit=0):
        ...
        ret = self._create_examples(
            ...
            PronounProcessor._read_txt(os.path.join(data_dir, "train.txt.block1")),
            "train")
        if limit:
            ret = ret[0:limit]
        return ret
    ...
    def get_dev_examples(self, data_dir, limit=0):
        ...
        ret = self._create_examples(
            ...
            PronounProcessor._read_txt(os.path.join(data_dir, "dev.txt.block1")),
            "dev")
        if limit:
            ret = ret[0:limit]
        return ret
    ...
    def get_test_examples(self, data_dir, limit=0):
        ...
        ret = self._create_examples(
            ...
            PronounProcessor._read_txt(os.path.join(data_dir, "test.txt.block1")),
            "test")
        if limit:
            ret = ret[0:limit]
        return ret
...
def run_evaluation(args, processor, label_list, tokenizer, output_mode, epoch, 
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        output_eval_file = os.path.join(args.output_dir, typ + "_"+str(epoch)+"_results.txt")
