...
def anonymize(words, task):
    if task == "pronoun-bert":
        gender_replacements = {
            "he": "they",
            "she": "they",
            "her": "their",
...
class SstWikiProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, task, anon, limit=0):
        """See base class."""
    ...
    def get_dev_examples(self, data_dir, task, anon, limit=0):
        """See base class."""
        ret = self._create_examples(
    ...
    def get_test_examples(self, data_dir, task, anon, limit=0):
        """See base class."""
        ret = self._create_examples(
    ...
    def _create_examples(self, lines, set_type, task, anon):
...
class PronounProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, block_lines, set_type, task, anon): 
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            ...
            #NEWLY ADDED
            if anon:
                text = anonymize(text, task)
                block = [0]*len(block) # Probably not necessary
            examples.append(
...
def attention_regularization_loss(attention_probs_layers, 
    ...
    if debug:
        print("Regularized attention mask:{}".format(reg_attention_mask))
        print("Non-Regular attention mask:{}".format(non_reg_attention_mask))
    # 32, 12, 1, 128
    # 32, 12, 1, 128 -> 32, 12, 1
...
def main():
    ...
    # NEWLY ADDED
    parser.add_argument('--anon', default=0, type=int, 
                        help="Set to 1 you want to anonymize, 0 if you dont.")
    ...
    base_labels = {}
    print("FIRST RUN:{}".format(args.first_run))
    ...
    # Define the processor type: sst-wiki or pronoun (works for occ-class too)
    ...
    if args.do_train:
        ...
        #### CHANGED THIS: Added task and anon as params
        train_examples = processor.get_train_examples(args.data_dir, args.name, args.anon, limit)
        print("length of train_examples is:", len(train_examples))
    ...
    nb_tr_steps = 0
    tr_loss = 0
    # Get the features, like mask etc. 
    if args.do_train:
...
def run_evaluation(args, processor, label_list, tokenizer, output_mode, epoch, 
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        limit = 2 if debug else 0
        if typ == "dev":
            eval_examples = processor.get_dev_examples(args.data_dir, args.name, args.anon, limit)
        else:
            eval_examples = processor.get_test_examples(args.data_dir, args.name, args.anon, limit)
