...
class SstWikiProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, limit=0):
        ...
        ret = self._create_examples(
            SstWikiProcessor._read_txt(os.path.join(data_dir, "train_1.txt")), "train")
        if limit:
            ret = ret[0:limit]
        return ret
    ...
    def get_dev_examples(self, data_dir, limit=0):
        """See base class."""
        ret = self._create_examples(
            SstWikiProcessor._read_txt(os.path.join(data_dir, "dev_1.txt")), "dev")
        if limit:
            ret = ret[0:limit]
        return ret
    ...
    def get_test_examples(self, data_dir, limit=0):
        """See base class."""
        ret = self._create_examples(
            SstWikiProcessor._read_txt(os.path.join(data_dir, "test_1.txt")), "test")
        if limit:
            ret = ret[0:limit]
        return ret
...
class PronounProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, limit=0):
        """See base class."""
        ret = self._create_examples(
            PronounProcessor._read_txt(os.path.join(data_dir, "train_1.txt")),
            PronounProcessor._read_txt(os.path.join(data_dir, "train_1.txt.block")),
            "train")
        if limit:
            ret = ret[0:limit]
        return ret
    ...
    def get_dev_examples(self, data_dir, limit=0):
        """See base class."""
        ret = self._create_examples(
            PronounProcessor._read_txt(os.path.join(data_dir, "dev_1.txt")),
            PronounProcessor._read_txt(os.path.join(data_dir, "dev_1.txt.block")),
            "dev")
        if limit:
            ret = ret[0:limit]
        return ret
    ...
    def get_test_examples(self, data_dir, limit=0):
        """See base class."""
        ret = self._create_examples(
            PronounProcessor._read_txt(os.path.join(data_dir, "test_1.txt")),
            PronounProcessor._read_txt(os.path.join(data_dir, "test_1.txt.block")),
            "test")
        if limit:
            ret = ret[0:limit]
        return ret
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # print(filepath)
        # df = pd.read_csv(
        #     'C:/Users/howar/Downloads/deceptive-attention-master/deceptive-attention-master/src/classification_tasks/pytorch-pretrained-BERT/examples/impermissible.csv')
...
def attention_regularization_loss(attention_probs_layers,
    ...
    return - hammer_coeff * hammer_reg, torch.mean(reg_attention_sum), torch.mean(
        non_reg_attention_sum), reg_attention_sum, torch.mean(pad_attention_sum), torch.max(
        reg_attention_sum), torch.max(non_reg_attention_sum), torch.argmax(non_reg_attention_sum, dim=1)
