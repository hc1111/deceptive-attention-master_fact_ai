...
class SSTProcessor(DataProcessor):
    """Processor for the SST data set (sentence level)."""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            tag, review = line.split(" ||| ")
            review = review.strip()
            if int(tag) == 2:  # ignore the neutral reviews
                continue

            text_a = review
            if int(tag) < 2: # negative reviews
                label = "0"
            elif int(tag) > 2: # positive reviews
                label = "1"
...
def compute_metrics(task_name, preds, labels):
    ...
    elif task_name == "sst-s":
        return {"acc": simple_accuracy(preds, labels)}
    elif task_name == "sst-wiki":
        return {"acc": simple_accuracy(preds, labels)}
    elif task_name == "mrpc":
...
def main():
    ...
    parser.add_argument('--base_label_file_dev',
                        type=str,
                        default="base_labels_dev.txt")
    parser.add_argument('--base_label_file_test',
                        type=str,
                        default="base_labels_test.txt")
    ...
    if not args.first_run:
        for typ in ["dev", "test"]:
            base_labels_content = open(vars(args)["base_label_file_" + typ], 'r').readlines()
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                logits, attention_probs_layers, category_mask, vnf = model(input_ids, 
                ...
                if output_mode == "classification":
                    ...
                    loss += attention_regularization_loss(attention_probs_layers, 
                                                            ...
                                                            input_mask,
                                                            args.hammer_coeff, 
                                                            optimize_func=args.att_opt_func)[0]
            ...
            for typ in ["dev", "test"]:
                eval_preds[typ] = run_evaluation(args, processor, label_list, tokenizer, output_mode, epoch, 
                                                    model, num_labels, tr_loss, global_step, device, task_name, 
                                                    base_labels, debug, typ)

        #dump labels after all the epochs
        for typ in ["dev", "test"]:
            preds = eval_preds[typ]
            labels_file = os.path.join(args.output_dir, typ + "_labels.txt")
            with open(labels_file, "w") as writer:
                logger.info("Dumping labels")
                writer.write('\n'.join( [ "{},{}".format(preds[i], all_label_ids.numpy()[i])
...
def get_vnf_aggregates(vnfs):
    # vnf - B X H
    d = -1; kd = False
    vnfs = [vnf.mean(d, kd), vnf.max(d, kd), vnf.min(d, kd) for vnf in vnfs]
    import pdb; pdb.set_trace()
    vnfs = [vnf.mean().item() for vnf in vnfs]
    return vnfs
...
def run_evaluation(args, processor, label_list, tokenizer, output_mode, epoch, 
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        print("typ\tepoch\tacc\tavg_mean_mass\tavg_max_mass\tloss\thammer_loss\tlabel_match_score")
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            vaggs = get_vnf_aggregates(vnf)
            tmp_vnfs = [tmp_vnf + vaggs[i] for i, tmp_vnf in enumerate(tmp_vnfs)]
        ...
        result['avg_mean_value_norm'] = tmp_vnfs[0] / nb_eval_steps
        result['avg_max_value_norm'] = tmp_vnfs[1] / nb_eval_steps
        result['avg_min_value_norm'] = tmp_vnfs[2] / nb_eval_steps
