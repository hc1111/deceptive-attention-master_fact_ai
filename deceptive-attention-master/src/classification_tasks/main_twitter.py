...
params={}
# parsing stuff fom the command line
parser = argparse.ArgumentParser(
    formatter_class=argparse.ArgumentDefaultsHelpFormatter)
params['seed']={1}

parser.add_argument('--emb-size', dest='emb_size', type=int, default=64,
...
parser.add_argument('--task', dest='task', default='pronoun',
        choices=('pronoun','twitter', 'sst', 'sst-wiki', 'sst-wiki-unshuff', 'reco', 'reco-rank', 'de-pronoun', 'de-refs', 'de-sst-wiki', 'occupation-classification', 'de-occupation-classification', 'occupation-classification_all'),
...
log.pr_blue("Coef (random-entropy): %0.2f" %(C_ENTROPY))
 ...
 # convert_examples_to_inputs(dev_texts, dev_labels, label2idx, MAX_SEQ_LENGTH, tokenizer)
...
def read_dataset(data_file, block_words=None, block_file=None, attn_file=None, clip_vocab=False):
    print(data_file)
    data_lines = open(data_file, encoding="utf8").readlines()
    global w2i
    ...
    for idx, data_line in enumerate(data_lines):
        if len(data_line.lower()[2:])>1:
            tag = data_line.lower()[0]
            words =data_line.lower()[2:]
            if TO_ANON:
            ...
            if block_words is not None:
                ...
                block_ids = [int(i) for i in block_lines[idx].split()]
...
def evaluate(dataset, iter, name='test', attn_stats=False, num_vis=0):
    ...
    for idx, words, block_ids, attn_orig , tag in dataset:
        if len(words)>1:
            if np.sum(block_ids)<len(words):
                words_t = torch.tensor([words]).type(type)
                tag_t = torch.tensor([tag]).type(type)
                tag_t=torch.nn.functional.one_hot(tag_t)
                ...
                ce_loss = calc_ce_loss(pred, torch.argmax(tag_t,1).long())
...
for ITER in range(1, NUM_EPOCHS+1):
    ...
    for num, (idx, words_orig, block_ids, attn_orig, tag) in enumerate(train):
        if len(words_orig)>2:
            if np.sum(block_ids)<len(words_orig):
                ...
                # print(model.weight_layer(torch.ones(model.embedding_layer(words).size()).to('cuda')))

                attention = attns[0]
                # print(out)
                # print(attention)
                ce_loss = calc_ce_loss(out, torch.argmax(out,1).long())
