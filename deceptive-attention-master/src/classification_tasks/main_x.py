...
def anonymize_x(words, taskname):
    if taskname == 'pronoun': 
        gender_replacements = {
            "he": "they",
            "she": "they",
...
print("Anonymized is ", TO_ANON)
print("block file ", USE_BLOCK_FILE)
print("attn file, ", USE_ATTN_FILE)
print("Blofk words are", BLOCK_WORDS)
...
def read_dataset(data_file, block_words=None, block_file=None, attn_file=None, clip_vocab=False):
    ...
    for idx, data_line in enumerate(data_lines):
        tag, words = data_line.strip().split("\t")
        if TO_ANON:
            if TASK_NAME == "sst-wiki": 
                words = words.split('[SEP]')[0]
            else: 
                words = anonymize_x(words, TASK_NAME)
